...
void NoneBorder::fillBackground(const Graphics& graphics, const int left, const int top, const int width, const int height) const {
	for(int i = 0 ; i < height ; i++ )
		for(int j = 0 ; j < width ; j++ )
			graphics.write( j + left, i + top, " ");
}

void NoneBorder::draw(const Graphics& graphics, const int left, const int top, const int width, const int height) const {
